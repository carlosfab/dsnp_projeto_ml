...
def download_one_file_of_raw_data(
    ...
    if response.status_code == 200:
        ...
        raise Exception(f'Failed to download {URL_PATH}')
...
def validate_raw_data(
    ...
    if 'pickup_datetime' not in rides.columns:
        raise ValueError(
            "The 'pickup_datetime' column is missing from the rides DataFrame.")
...
def load_raw_data(
    ...
    if months is None:
        ...
        # download data only for the months specified
        months = [months]
    ...
    for month in months:
        ...
        if not local_file.exists():
            try:
                # if not downloaded yet, download it from the NYC Taxi & Limousine Commission website
            ...
            print(f'{year}-{month:02d} was already downloaded.')
...
def add_missing_slots(agg_rides: pd.DataFrame) -> pd.DataFrame:
    ...
    for location_id in tqdm(location_ids):
        agg_rides_i = agg_rides.loc[agg_rides['pickup_location_id'] == location_id, [
            'pickup_hour', 'rides']]
...
def transform_raw_data_into_ts_data(
    ...
    # add missing rows with 0 zero rides
...
def transform_ts_data_into_training_data(
